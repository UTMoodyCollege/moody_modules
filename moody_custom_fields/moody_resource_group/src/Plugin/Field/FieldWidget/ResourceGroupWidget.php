...
namespace Drupal\moody_resource_group\Plugin\Field\FieldWidget;
...
/**
 * Plugin implementation of the 'resource_group_widget' widget.
 *
 * @FieldWidget(
 *   id = "resource_group_widget",
 *   module = "moody_resource_group",
 *   label = @Translation("Resource group widget"),
 *   field_types = {
 *     "resource_group"
 *   }
 * )
 */
class ResourceGroupWidget extends WidgetBase {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $links = isset($items[$delta]->links) ? (array) unserialize($items[$delta]->links) : [];
    if ($link_count === NULL) {
      if (empty($links)) {
        $link_count = 1;
      }
