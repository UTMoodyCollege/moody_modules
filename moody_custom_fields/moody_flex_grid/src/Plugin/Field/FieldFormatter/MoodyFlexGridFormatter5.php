 ...
 *
 * @FieldFormatter(
 *   id = "moody_flex_grid_formatter5",
 *   label = @Translation("Flip Style"),
 *   field_types = {
 *     "moody_flex_grid"
 *   }
 * )
 */
class MoodyFlexGridFormatter5 extends FormatterBase implements ContainerFactoryPluginInterface {
  /**
    ...
    foreach ($items as $delta => $item) {
      ...
      if (!empty($flex_grid_items)) {
        foreach ($flex_grid_items as $key => $instance) {
          ...
          if (!empty($instance_item['headline_color'])) {
            $instances[$key]['headline_color'] = $instance_item['headline_color'];
          }
          if (!empty($instance_item['copy'])) {
            $instances[$key]['copy'] = check_markup($instance_item['copy'], $instance_item['copy_format'] ?? 'flex_html');
          }
      ...
      $elements['#attributes']['class'][] = 'moody-flex-grid-wrapper rectangular-display';
    }
    $elements['#attached']['library'][] = 'moody_flex_grid/moody-flex-grid';
    // add the moody-flex-grid-flip-card too
    $elements['#attached']['library'][] = 'moody_flex_grid/moody-flex-grid-flip-card';
    return $elements;
  }
