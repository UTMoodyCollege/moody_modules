...
class MoodyFeaturedHighlight {
  ...
  public static function getRawSourceData($source_nid) {
    ...
    $source_data = Database::getConnection()->select('field_data_field_moody_featured_highlight_a', 'f')
      ->fields('f')
    ...
    foreach ($source_data as $delta => $item) {
      $prepared[$delta] = [
        'type' => $item->bundle,
        'image_fid' => $item->field_moody_featured_highlight_a_image_fid,
        'date' => $item->field_moody_featured_highlight_a_date,
        'headline' => $item->field_moody_featured_highlight_a_headline,
        'copy' => $item->field_moody_featured_highlight_a_copy_value,
        'link_href' => $item->field_moody_featured_highlight_a_link,
        'link_title' => $item->field_moody_featured_highlight_a_cta,
        'style' => $item->field_moody_featured_highlight_a_highlight_style,
      ];
    }
    return $prepared;
  }
